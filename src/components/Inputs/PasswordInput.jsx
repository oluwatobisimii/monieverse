...
const PasswordInput = ({ value, onChange, label, placeholder, errors }) => {
  ...
  return (
    <>
      <div className="w-full">
        ...
        <div className="h-1" />
        <div className={`flex items-center border  rounded-lg focus-within:border-primary-400 focus-within:shadow-[0px_0px_0px_3px_#DDD7FE] ${
            errors && errors.password ? "border-error-400" : "border-gray-100"
          }`}>
          <input
            type={passwordType}
            name={"password"}
      ...
      {errors && (
        <p className="text-xs  text-error-400">{errors.password}</p>
      )}
    </>
  );
};
